...
let createUser = async (req, res, next) => {
  try {
    ...
    await Designation.create({f_Designation})
...
if(imagePath.includes('.jpg') || imagePath.includes('.pdf') ||imagePath.includes('.png') ||
imagePath.includes('.JPG') || imagePath.includes('.PDF') ||imagePath.includes('.PNG') ){
...
res
    .status(201)
    .json({ error: false, message: "file should only jpg/pdf/png" });
...
let updateUser = async (req, res, next) => {
  try {
    ...
    if (!req.file) {
      ...
      res
        .status(200)
        .json({ error: false, message: "data updated successfully", user });
    }
    ...
    res.status(400).json({error:true,message:'unexpected error'})
  } catch (err) {
    next(err);
  }
};
